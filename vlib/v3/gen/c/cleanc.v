...
fn (mut g FlatGen) preamble() {
	...
	g.writeln('typedef ${g.tc.int_c_type()} int_literal;')
...
fn (mut g FlatGen) builtin_compat_decls() {
	...
	g.writeln('void array__push_many(array* a, void* val, ${g.tc.int_c_type()} size);')
	...
	g.writeln('static inline ${int_type} array_index_int(Array a, ${int_type} val) { for (int i = 0; i < a.len; i++) { if (a.elem_size == sizeof(i64)) { if (((i64*)a.data)[i] == (i64)val) return (${int_type})i; } else if (((i32*)a.data)[i] == (i32)val) return (${int_type})i; } return (${int_type})-1; }')
	g.writeln('static inline bool array_contains_int(Array a, ${int_type} val) { return array_index_int(a, val) >= 0; }')
	...
	g.writeln('static inline bool fixed_array_contains_int(const ${int_type}* a, int len, ${int_type} val) { for (int i = 0; i < len; i++) if (a[i] == val) return true; return false; }')
