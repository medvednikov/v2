...
fn (mut g FlatGen) gen_decl_or_expr(lhs flat.Node, or_node flat.Node) {
	...
	g.writeln('} else {')
	g.tc.push_scope()
	g.tc.cur_scope.insert('err', types.Type(types.string_))
	g.indent++
	g.writeln('string err = (string){0};')
...
fn (mut g FlatGen) gen_or_expr(node flat.Node) {
	...
	g.write('({${opt_ct} ${tmp} = ')
	g.gen_expr(expr_id)
	g.write('; ${tmp}.ok ? ${tmp}.value : ({string err = (string){0}; (void)err; ')
...
fn (mut g FlatGen) optional_type_name(t types.Type) string {
	base_type := match t {
		types.OptionType { t.base_type }
		types.ResultType { t.base_type }
		else { return g.tc.c_type(t) }
	}
...
fn (mut g FlatGen) emit_const(name string, val_id flat.NodeId) {
	...
	if !g.is_const_expr(val_id) {
		...
		if v_type is types.Struct && v_type.name.starts_with('C.') {
			return
		}
		ct := g.tc.c_type(v_type)
		if ct != 'int' && ct != 'void' {
