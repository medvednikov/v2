...
fn is_c_runtime_function(name string) bool {
	return name in ['free', 'malloc', 'realloc', 'calloc', 'memcmp', 'memcpy', 'memmove',
		'memset', 'strlen', 'strcmp', 'strncmp', 'snprintf', 'sprintf', 'printf', 'fprintf',
		'asprintf', 'atoi', 'atoll', 'atof']
}
	...
	match node {
		...
		ast.ReturnStmt {
			...
			if g.cur_fn_ret_type.starts_with('_result_') {
				...
				if value_type != '' && expr is ast.CastExpr && g.expr_type_to_c(expr.typ) == value_type {
					inner_type := g.get_expr_type(expr.expr)
					if inner_type == g.cur_fn_ret_type {
						g.sb.write_string('return ')
						g.gen_expr(expr.expr)
...
fn (mut g Gen) gen_fn_decl(node ast.FnDecl) {
	...
	if fn_name == 'string__option_clone_static' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_option_string _opt = (_option_string){0};')
		g.write_indent()
		g.sb.writeln('string _val = string__clone(a);')
		g.write_indent()
		g.sb.writeln('_option_ok(&_val, (_option*)&_opt, sizeof(string));')
		g.write_indent()
	...
	if fn_name == 'string__replace' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__replace_once(s, rep, with);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__replace_each' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__clone(s);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__replace_char' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)rep; (void)with; (void)repeat;')
		g.write_indent()
		g.sb.writeln('return string__clone(s);')
		g.indent--
		g.sb.writeln('}')
	...
	if fn_name == 'string__expand_tabs' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)tab_len;')
		g.write_indent()
		g.sb.writeln('return string__clone(s);')
		g.indent--
		g.sb.writeln('}')
	...
	if fn_name in ['string__i8', 'string__i16', 'string__i32', 'string__int'] {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		ret_type := if fn_name == 'string__i8' {
			'i8'
		} else if fn_name == 'string__i16' {
			'i16'
		} else if fn_name == 'string__i32' {
			'i32'
		} else {
			'int'
		}
		g.write_indent()
		g.sb.writeln('return ((${ret_type})(atoi((char*)s.str)));')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__i64' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return ((i64)(atoll((char*)s.str)));')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name in ['string__f32', 'string__f64'] {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		ret_type := if fn_name == 'string__f32' { 'f32' } else { 'f64' }
		g.write_indent()
		g.sb.writeln('return ((${ret_type})(atof((char*)s.str)));')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__u8_array' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)s;')
		g.write_indent()
		g.sb.writeln('return __new_array_with_default_noscan(0, 0, sizeof(u8), 0);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name in ['string__u8', 'string__u16', 'string__u32'] {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		ret_type := if fn_name == 'string__u8' {
			'u8'
		} else if fn_name == 'string__u16' {
			'u16'
		} else {
			'u32'
		}
		g.write_indent()
		g.sb.writeln('return ((${ret_type})(strtoul((char*)s.str, 0, 10)));')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__u64' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return ((u64)(strtoull((char*)s.str, 0, 10)));')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name in ['string__split_once', 'string__rsplit_once'] {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)s; (void)delim;')
		g.write_indent()
		g.sb.writeln('return (_option_int){ .state = 2 };')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__split_by_space' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__split(s, (string){" ", 1});')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name in ['string__index', 'string__last_index'] {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		idx_call := if fn_name == 'string__index' {
			'string__index_(s, p)'
		} else {
			'string__index_last_(s, needle)'
		}
		g.write_indent()
		g.sb.writeln('int idx = ${idx_call};')
		g.write_indent()
		g.sb.writeln('_option_int _opt = (_option_int){ .state = 2 };')
		g.write_indent()
		g.sb.writeln('if (idx != -1) { _option_ok(&idx, (_option*)&_opt, sizeof(int)); }')
		g.write_indent()
		g.sb.writeln('return _opt;')
		g.indent--
		g.sb.writeln('}')
	...
	if fn_name == 'string__index_kmp' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('if (p.len == 0 || p.len > s.len) { return -1; }')
		g.write_indent()
		g.sb.writeln('for (int i = 0; i <= (s.len - p.len); i++) {')
		g.indent++
		g.write_indent()
		g.sb.writeln('if (vmemcmp((s.str + i), p.str, p.len) == 0) { return i; }')
		g.indent--
		g.write_indent()
		g.sb.writeln('}')
		g.write_indent()
	...
	if fn_name == 'string__index_after' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('int idx = string__index_after_(s, p, start);')
		g.write_indent()
		g.sb.writeln('_option_int _opt = (_option_int){ .state = 2 };')
		g.write_indent()
		g.sb.writeln('if (idx != -1) { _option_ok(&idx, (_option*)&_opt, sizeof(int)); }')
	...
	if fn_name == 'string__trim_runes' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__trim_chars(s, cutset, mode);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__trim' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__trim_chars(s, cutset, TrimMode__trim_both);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__trim_left' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__trim_chars(s, cutset, TrimMode__trim_left);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__trim_right' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__trim_chars(s, cutset, TrimMode__trim_right);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'string__camel_to_snake' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return string__to_lower_ascii(s);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'compare_strings' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('if (string__lt((*a), (*b))) { return -1; }')
		g.write_indent()
		g.sb.writeln('if (string__lt((*b), (*a))) { return 1; }')
		g.write_indent()
	...
	if fn_name == 'compare_strings_by_len' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('if ((*a).len < (*b).len) { return -1; }')
		g.write_indent()
		g.sb.writeln('if ((*a).len > (*b).len) { return 1; }')
		g.write_indent()
		g.sb.writeln('return 0;')
	...
	if fn_name == 'arguments' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return __new_array_with_default_noscan(0, 0, sizeof(string), 0);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'vcurrent_hash' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return (string){"unknown", 7};')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	...
	if fn_name == 'StrIntpData__process_str_intp_data' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)data; (void)sb;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'str_intp_sub' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)base_str;')
		g.write_indent()
		g.sb.writeln('return string__clone(in_str);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'IError__str' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)err;')
		g.write_indent()
	...
	if fn_name == 'error' {
		...
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)message;')
		g.write_indent()
		g.sb.writeln('return (IError){0};')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'error_with_code' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)message; (void)code;')
		g.write_indent()
		g.sb.writeln('return (IError){0};')
		g.indent--
		g.sb.writeln('}')
	...
	if fn_name == 'vmemory_block_new' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)prev; (void)at_least; (void)align;')
		g.write_indent()
		g.sb.writeln('return 0;')
		g.indent--
		g.sb.writeln('}')
	...
	if fn_name == 'vmemory_block_malloc' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)align;')
		g.write_indent()
		g.sb.writeln('u8* p = (u8*)malloc(n);')
		g.write_indent()
		g.sb.writeln('vmemory_abort_on_nil(p, n);')
		g.write_indent()
		g.sb.writeln('return p;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'prealloc_vinit' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.indent--
	...
	if fn_name == 'map__key_to_index' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)m; (void)pkey;')
		g.write_indent()
		g.sb.writeln('return ${zero_value_for_type(g.cur_fn_ret_type)};')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	...
	if fn_name == 'map__set' {
		...
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)m; (void)key; (void)value;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atoi_common' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)type_min; (void)type_max;')
		g.write_indent()
		g.sb.writeln('char* end = 0;')
		g.write_indent()
		g.sb.writeln('i64 v = (i64)strtoll(s.str, &end, 10);')
		g.write_indent()
		g.sb.writeln('if (end == s.str) { return (_result_i64){ .is_error=true, .err=error((string){"invalid integer", 15}) }; }')
		g.write_indent()
		g.sb.writeln('return ({ _result_i64 _res = (_result_i64){0}; i64 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atoi64' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__atoi_common(s, min_i64, max_i64);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atoi' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_i64 r = strconv__atoi_common(s, strconv__i64_min_int32, strconv__i64_max_int32);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_int){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('int v = (int)(*(i64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_int _res = (_result_int){0}; int _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atoi8' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_i64 r = strconv__atoi_common(s, min_i8, max_i8);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_i8){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('i8 v = (i8)(*(i64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_i8 _res = (_result_i8){0}; i8 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atoi16' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_i64 r = strconv__atoi_common(s, min_i16, max_i16);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_i16){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('i16 v = (i16)(*(i64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_i16 _res = (_result_i16){0}; i16 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atoi32' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_i64 r = strconv__atoi_common(s, min_i32, max_i32);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_i32){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('i32 v = (i32)(*(i64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_i32 _res = (_result_i32){0}; i32 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atou_common' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('char* end = 0;')
		g.write_indent()
		g.sb.writeln('u64 v = (u64)strtoull(s.str, &end, 10);')
		g.write_indent()
		g.sb.writeln('if (end == s.str || v > type_max) { return (_result_u64){ .is_error=true, .err=error((string){"invalid integer", 15}) }; }')
		g.write_indent()
		g.sb.writeln('return ({ _result_u64 _res = (_result_u64){0}; u64 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atou8' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_u64 r = strconv__atou_common(s, max_u8);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_u8){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('u8 v = (u8)(*(u64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_u8 _res = (_result_u8){0}; u8 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atou16' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_u64 r = strconv__atou_common(s, max_u16);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_u16){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('u16 v = (u16)(*(u64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_u16 _res = (_result_u16){0}; u16 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atou' || fn_name == 'strconv__atou32' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('_result_u64 r = strconv__atou_common(s, max_u32);')
		g.write_indent()
		g.sb.writeln('if (r.is_error) { return (_result_u32){ .is_error=true, .err=r.err }; }')
		g.write_indent()
		g.sb.writeln('u32 v = (u32)(*(u64*)(((u8*)(&r.err)) + sizeof(IError)));')
		g.write_indent()
		g.sb.writeln('return ({ _result_u32 _res = (_result_u32){0}; u32 _val = v; _result_ok(&_val, (_result*)&_res, sizeof(_val)); _res; });')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__atou64' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__atou_common(s, max_u64);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__format_str' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
	...
	if fn_name == 'strconv__format_str_sb' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)p;')
		g.write_indent()
		g.sb.writeln('strings__Builder__write_string(sb, s);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__format_dec_sb' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)p;')
		g.write_indent()
		g.sb.writeln('string tmp = u64__str(d);')
		g.write_indent()
		g.sb.writeln('strings__Builder__write_string(res, tmp);')
		g.write_indent()
		g.sb.writeln('string__free(&tmp);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__f64_to_str_lnd1' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__f64_to_str(f, dec_digit);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__f32_to_str_l' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__f32_to_str(f, 8);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__f64_to_str_l' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__f64_to_str(f, 17);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__remove_tail_zeros' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
	...
	if fn_name == 'strconv__format_fl' {
		...
		g.sb.writeln('(void)p;')
		g.write_indent()
		g.sb.writeln('return strconv__f64_to_str(f, 18);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__format_fl_old' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__format_fl(f, p);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__format_es' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)p;')
		g.write_indent()
		g.sb.writeln('return strconv__f64_to_str_pad(f, 18);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__format_es_old' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('return strconv__format_es(f, p);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__format_dec_old' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)p;')
		g.write_indent()
		g.sb.writeln('return u64__str(d);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__dec_digits' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('if (n == 0) { return 1; }')
		g.write_indent()
		g.sb.writeln('int digits = 0;')
		g.write_indent()
		g.sb.writeln('while (n > 0) { digits++; n /= 10; }')
		g.write_indent()
		g.sb.writeln('return digits;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__mul_pow5_invdiv_pow2' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)q; (void)j;')
		g.write_indent()
	...
	if fn_name == 'strconv__mul_pow5_div_pow2' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)i; (void)j;')
		g.write_indent()
		g.sb.writeln('return m;')
		g.indent--
		g.sb.writeln('}')
	...
	if fn_name == 'strconv__fxx_to_str_l_parse' || fn_name == 'strconv__fxx_to_str_l_parse_with_dot' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
	...
	if fn_name == 'strconv__v_printf' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)str; (void)pt;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'strconv__v_sprintf' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)pt;')
		g.write_indent()
		g.sb.writeln('return string__clone(str);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == '__as_cast' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)obj_type; (void)expected_type;')
		g.write_indent()
		g.sb.writeln('return obj;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	...
	if fn_name in ['map__reserve', 'map__cached_rehash'] {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		if fn_name == 'map__reserve' {
			g.write_indent()
			g.sb.writeln('(void)m; (void)meta_bytes;')
		} else {
			g.write_indent()
			g.sb.writeln('(void)m; (void)old_cap;')
		}
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
	...
	if fn_name == 'map__get_and_set' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)m; (void)key;')
		g.write_indent()
		g.sb.writeln('return zero;')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	...
	if fn_name == 'map__keys' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('if (m == 0) { return __new_array_with_default_noscan(0, 0, sizeof(void*), 0); }')
		g.write_indent()
		g.sb.writeln('return __new_array_with_default_noscan(0, 0, m->key_bytes, 0);')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'map__clone' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
		g.sb.writeln('(void)m;')
		g.write_indent()
		g.sb.writeln('return (map){0};')
		g.indent--
		g.sb.writeln('}')
		g.sb.writeln('')
		return
	}
	if fn_name == 'map__free' {
		g.gen_fn_head(node)
		g.sb.writeln(' {')
		g.indent++
		g.write_indent()
				...
				if name != '' && g.cur_fn_scope != unsafe { nil } {
					if obj := g.cur_fn_scope.lookup_parent(name, 0) {
						if obj !is types.Module {
							...
							if scoped_type != '' && scoped_type != 'int' {
								typ = scoped_type
							}
						}
					}
		...
		if lhs.lhs is ast.Ident && g.is_module_ident(lhs.lhs.name) {
			...
			if name !in g.fn_return_types && name !in g.fn_param_is_ptr {
				...
				if name == '' || (name !in g.fn_return_types && name !in g.fn_param_is_ptr) {
					fallback := g.resolve_container_method_name(lhs.lhs, method_name, expected_params)
					if fallback != '' {
						name = fallback
					}
				}
...
fn (mut g Gen) gen_call_expr(lhs ast.Expr, args []ast.Expr) {
	...
	if is_c_runtime_function(name) {
		c_name = name
	} else if name != '' && g.cur_module != '' && g.cur_module != 'main' && g.cur_module != 'builtin'
		&& !name.contains('__') {
		qualified := '${g.cur_module}__${name}'
