	...
	match node {
		...
		ast.CallExpr {
			...
			if name.starts_with('__Map_') && name.ends_with('_get_check') {
				// Extract value type from Map_K_V_get_check -> V*
				parts := name[6..name.len - 10].split('_') // Remove __Map_ and _get_check
				if parts.len >= 2 {
					value_type := parts[parts.len - 1] // Last part is value type
					return '${value_type}*'
				}
			}
...
fn (mut g Gen) gen_fn_decl(node ast.FnDecl) {
	...
	if g.cur_module in ['cleanc', 'v', 'ssa', 'x64', 'optimize', 'transformer', 'types', 'token', 'parser', 'scanner', 'binary', 'sha256', 'term']
	...
	match node {
		...
		ast.Ident {
			...
			if node.name.starts_with('@') {
				...
				variant_full := g.cur_sumtype_match_variant
				// Extract simple variant name for _data._ accessor (strip module prefix)
				variant_simple := if variant_full.contains('__') {
					parts := variant_full.split('__')
					parts[parts.len - 1] // Last part is the simple name
				} else {
					variant_full
				}
				// The full variant name is already correctly qualified, use it for type cast
				variant_mangled := if variant_full.contains('__') {
					variant_full // Already has module prefix
				} else if g.cur_sumtype_match_type.contains('__') {
					// Derive module from sum type
					parts := g.cur_sumtype_match_type.split('__')
					if parts.len >= 2 {
						'${parts[0]}__${variant_full}'
					} else {
						variant_full
					}
				} else {
					g.mangle_type_if_needed(variant_full)
				}
				if variant_simple in ['int', 'i64', 'i32', 'i16', 'i8', 'u64', 'u32', 'u16', 'u8', 'byte',
					'rune', 'bool', 'f32', 'f64', 'usize', 'isize'] {
					g.sb.write_string('((${variant_simple})(intptr_t)${node.name}._data._${variant_simple})')
				} else if variant_simple == 'string' {
					// String is a struct - was stored as pointer
					g.sb.write_string('(*(string*)${node.name}._data._${variant_simple})')
				} else {
					// Struct type - access via pointer cast, using mangled variant name
					g.sb.write_string('(*(${variant_mangled}*)${node.name}._data._${variant_simple})')
				}
