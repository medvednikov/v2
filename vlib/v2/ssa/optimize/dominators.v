	...
	for func in m.funcs {
		...
		for i := ctx.n; i >= 2; i-- {
			...
			for p in m.blocks[w].preds {
				...
				semi_u := ctx.semi[u]
				semi_w := ctx.semi[w]
				if semi_u < 0 || semi_u >= ctx.dfnum.len || semi_w < 0
					|| semi_w >= ctx.dfnum.len {
					continue
				}
...
fn (mut ctx LTContext) compress(v int) {
	...
	if ctx.ancestor[av] != -1 {
		...
		label_v := ctx.label[v]
		if label_av < 0 || label_av >= ctx.semi.len || label_v < 0
			|| label_v >= ctx.semi.len {
			return
		}
...
fn (mut ctx LTContext) eval(v int) int {
	...
	if semi_lav < 0 || semi_lav >= ctx.dfnum.len || semi_lv < 0
		|| semi_lv >= ctx.dfnum.len {
		return label_v
	}
